...
# Validate before evaluating the flake, replacing hardware config, or installing.
if [[ "$target_root" != /* ]] || ! target_root="$(realpath -e -- "$target_root")"; then
  echo "Target root must be an existing absolute directory." >&2
  exit 1
fi
if [[ ! -d "$target_root" || "$target_root" -ef / || "$repo_root" == "$target_root" || "$repo_root" == "$target_root/"* ]]; then
  echo "Refusing unsafe installation root: $target_root" >&2
  exit 1
fi
if ! findmnt --mountpoint "$target_root" >/dev/null 2>&1; then
...
if ! host="$(nix --extra-experimental-features "nix-command flakes" eval --raw "$repo_root#nixosConfigurations.\"$profile\".config.networking.hostName" 2>/dev/null)"; then
