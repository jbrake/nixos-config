...
# docs/vm-guests.md for the virt-manager side (UEFI firmware,
# virtio video, GL toggles).
{
  ...
  options.jbrake.spiceSessionWorkaround.enable = lib.mkEnableOption "custom SPICE session agent startup and duplicate-agent prevention";
  ...
  config = {
    ...
    environment.etc."xdg/autostart/spice-vdagent.desktop" =
      lib.mkIf config.jbrake.spiceSessionWorkaround.enable
        {
          text = ''
            [Desktop Entry]
            Type=Application
    ...
    # unit instead.
    systemd.user.services.spice-vdagent = lib.mkIf config.jbrake.spiceSessionWorkaround.enable {
